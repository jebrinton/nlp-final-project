...
with open("tatoeba/eng.txt", "r", encoding="utf-8") as src_file:
    source_sentences = src_file.read().splitlines()

with open("tatoeba/esp.txt", "r", encoding="utf-8") as tgt_file:
...
def calculate_percentage(df, word, palabra):
    ...
    filtered_df = df[df['eng_tokens'].apply(lambda tokens: word in tokens)]

    # print(filtered_df)

    # for row in range(len(df)):
    #     if word in df['eng_tokens'][row]:
    #         print(f"{word} in row {row}")
    ...
    count = filtered_df['esp_tokens'].apply(lambda tokens: palabra in tokens).sum()
...
sampled_df['random_word_en'] = sampled_df['eng_tokens'].apply(get_random_word)
sampled_df['random_word_es'] = sampled_df['esp_tokens'].apply(get_random_word)
